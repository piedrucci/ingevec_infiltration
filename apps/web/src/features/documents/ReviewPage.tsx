...
export function ReviewPage() {
  ...
  const candidateColumns = useMemo<ColumnDef<StockFeatures, DocumentCandidate, unknown>[]>(() => [
    {
      ...
      header: "",
      enableSorting: false,
      cell: ({ row }) => <input aria-label={`Seleccionar ítem ${row.original.postventa_item_id}`} type="checkbox" checked={selected.includes(row.original.postventa_item_public_id)} onChange={(event) => toggleItem(row.original.postventa_item_public_id, event.target.checked)} />,
    },
    { accessorKey: "project_id", header: "Obra", enableSorting: false, cell: ({ row }) => `${row.original.project_id} · ${row.original.project_name}` },
    { accessorKey: "postventa_item_id", header: "Ítem", enableSorting: false },
  ...
  return <section className="review-layout">
    <section className="card"><div className="section-title"><div><p className="eyebrow">DOCUMENTO</p><h2>{document.original_filename}</h2></div><DocumentStatusBadge status={document.status as DocumentStatus} /></div><div className="detail-grid"><div><strong>Cargado</strong><p>{new Intl.DateTimeFormat("es-CL", { dateStyle: "medium", timeStyle: "short" }).format(new Date(document.uploaded_at))}</p></div><div><strong>Proyecto detectado</strong><p>{document.extracted_data?.project_number?.match(/^\s*(\d+)/)?.[1] || "No detectado"} {document.extracted_data?.project_name || ""}</p></div><div><strong>Lugar</strong><p>{document.extracted_data?.infiltration_location || "No detectado"}</p></div><div className="detail-grid-full"><strong>Causa extraída</strong><p>{document.extracted_failure_cause || "No detectada"}</p></div></div><button type="button" disabled={isOpeningPdf} onClick={() => void openPdf()}>{isOpeningPdf ? <LoadingIndicator label="Abriendo PDF…" compact /> : "Abrir PDF"}</button>{document.processing_error && <ErrorMessage error={new Error(document.processing_error)} />}</section>
      <section className="card"><div className="section-title"><h2>Asociaciones</h2><span>{document.associations.length}</span></div><ErrorMessage error={removeAssociationMutation.error} />{document.associations.length ? <ul className="association-list">{document.associations.map((association) => <li key={association.postventa_item_public_id}><strong>{association.project_id} · Ítem {association.postventa_item_id}</strong><span>{association.notes}</span><div>{association.failure_causes.length ? association.failure_causes.map((cause) => <span className="cause-tag" key={cause.code}>{cause.display_name_es}</span>) : <small>Sin causa asociada</small>}</div><small>{association.association_source === "MANUAL" ? "Manual" : "Automática"}</small><button className="icon-danger-action" type="button" aria-label={`Desasociar ítem ${association.postventa_item_id}`} title="Desasociar ítem" disabled={removeAssociationMutation.isPending} onClick={() => { if (window.confirm("¿Desasociar este ítem del documento? El PDF no se eliminará.")) removeAssociationMutation.mutate(association); }}>{removeAssociationMutation.isPending && removeAssociationMutation.variables?.postventa_item_public_id === association.postventa_item_public_id ? <LoadingIndicator label="Desasociando…" compact /> : <svg aria-hidden="true" viewBox="0 0 24 24" focusable="false"><path d="m10 14 4-4" /><path d="m8.5 8.5-1-1a3 3 0 0 0-4.25 4.25l2.5 2.5A3 3 0 0 0 10 14.5l1-1" /><path d="m15.5 15.5 1 1a3 3 0 0 0 4.25-4.25l-2.5-2.5A3 3 0 0 0 14 9.5l-1 1" /></svg>}</button></li>)}</ul> : <p className="empty-state">Aún no existen asociaciones.</p>}</section>
    {canReview && <section className="card"><div className="section-title"><div><p className="eyebrow">REVISIÓN MANUAL</p><h2>Seleccionar ítems y causas</h2></div><span>{candidates.length} candidatos</span></div><ErrorMessage error={candidatesQuery.error ?? causesQuery.error ?? categoriesQuery.error ?? searchedItemsQuery.error ?? mutation.error ?? createCauseMutation.error} /><div className="review-form"><label>Causas normalizadas<select multiple value={causeCodes} onChange={(event) => setCauseCodes(Array.from(event.target.selectedOptions, (option) => option.value))}>{causesQuery.data?.map((cause) => <option key={cause.code} value={cause.code}>{cause.category_name_es} · {cause.display_name_es}</option>)}</select></label><p className="muted">Puedes seleccionar varias causas manteniendo presionada la tecla Cmd/Ctrl.</p><button className="secondary inline-action" type="button" disabled={createCauseMutation.isPending} onClick={() => { setIsAddingCause((current) => !current); if (!isAddingCause) setNewCauseAliases(document.extracted_failure_cause ?? ""); }}>{isAddingCause ? "Cancelar nueva causa" : "Agregar nueva causa"}</button>{isAddingCause && <fieldset className="new-cause-form"><legend>Nueva causa normalizada</legend><label>Nombre visible en español<input value={newCauseName} onChange={(event) => { setNewCauseName(event.target.value); setNewCauseCode(suggestedCode(event.target.value)); }} placeholder="Ej. Sellos desprendidos" /></label><label>Código para BI<input value={newCauseCode} onChange={(event) => setNewCauseCode(suggestedCode(event.target.value))} placeholder="SELLOS_DESPRENDIDOS" /></label><label>Categoría<select value={newCauseCategory} onChange={(event) => setNewCauseCategory(event.target.value)}><option value="">Selecciona una categoría</option>{categoriesQuery.data?.map((category) => <option key={category.code} value={category.code}>{category.display_name_es}</option>)}</select></label><label>Alias de detección (separados por coma)<input value={newCauseAliases} onChange={(event) => setNewCauseAliases(event.target.value)} placeholder="sellos desprendidos, sello suelto" /></label><button type="button" disabled={!newCauseName || !newCauseCode || !newCauseCategory || createCauseMutation.isPending} onClick={() => createCauseMutation.mutate()}>{createCauseMutation.isPending ? <LoadingIndicator label="Creando causa…" compact /> : "Crear y seleccionar causa"}</button></fieldset>}<label>Buscar otro ítem libre<input value={itemSearch} onChange={(event) => { setItemSearch(event.target.value); setCandidatePagination((current) => ({ ...current, pageIndex: 0 })); }} placeholder="Obra, proyecto u observación" /></label>{(candidatesQuery.isFetching || searchedItemsQuery.isFetching) && <LoadingIndicator label="Buscando ítems…" compact />}<div className="table-wrap"><table><thead>{candidateTable.getHeaderGroups().map((headerGroup) => <tr key={headerGroup.id}>{headerGroup.headers.map((header) => <th key={header.id}>{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</th>)}</tr>)}</thead><tbody>{candidateTable.getRowModel().rows.map((row) => <tr key={row.id}>{row.getVisibleCells().map((cell) => <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}{!candidateTable.getRowModel().rows.length && <tr><td colSpan={candidateColumns.length}>No hubo candidatos. Busca un ítem libre para asociarlo manualmente.</td></tr>}</tbody></table></div><div className="pagination"><label>Filas por página<select value={candidatePagination.pageSize} onChange={(event) => setCandidatePagination({ pageIndex: 0, pageSize: Number(event.target.value) })}><option value={15}>15</option><option value={25}>25</option><option value={50}>50</option></select></label><button className="secondary" type="button" disabled={!candidateTable.getCanPreviousPage()} onClick={() => candidateTable.previousPage()}>Anterior</button><span>{candidates.length ? `${candidatePagination.pageIndex * candidatePagination.pageSize + 1}–${Math.min((candidatePagination.pageIndex + 1) * candidatePagination.pageSize, candidates.length)} de ${candidates.length}` : "0 candidatos"}</span><button className="secondary" type="button" disabled={!candidateTable.getCanNextPage()} onClick={() => candidateTable.nextPage()}>Siguiente</button></div><button type="button" disabled={!selected.length || !chosenCauses.length || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? <LoadingIndicator label="Guardando asociación…" compact /> : "Confirmar asociación manual"}</button></div></section>}
