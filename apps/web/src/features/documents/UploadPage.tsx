...
export function UploadPage() {
  ...
  return <section className="card page-card"><div className="section-title"><div><p className="eyebrow">DOCUMENTOS</p><h2>Cargar informes PDF</h2></div><span>Máximo 5 MB por archivo</span></div><div className="upload-area"><p>Selecciona uno o más informes de filtración. Los archivos válidos se procesarán en segundo plano.</p><input aria-label="Seleccionar archivos PDF" type="file" accept="application/pdf,.pdf" multiple disabled={uploading} onChange={(event) => { const files = Array.from(event.target.files ?? []); if (files.length) void uploadFiles(files); event.currentTarget.value = ""; }} /><p className="muted">No se aceptará contenido duplicado, aunque tenga otro nombre.</p></div>{results.length > 0 && <div className="upload-results"><h3>Resultado del lote</h3>{results.map((result) => <div className={`upload-result ${result.outcome}`} key={result.id}><strong>{result.filename}</strong><span>{result.message}</span>{result.documentId && <Link to={`/documents/${result.documentId}/review`}>{result.outcome === "duplicate" ? "Ver documento existente" : "Ver estado"}</Link>}</div>)}</div>}<p><Link to="/documents">Ver historial de documentos</Link></p></section>;
}
