...
export function DocumentsPage() {
  ...
  return <section className="card"><div className="section-title"><div><p className="eyebrow">DOCUMENTOS</p><h2>Historial de cargas</h2></div><Link className="button-link" to="/documents/upload">Cargar PDFs</Link></div><div className="filters"><label>Estado <select value={status} onChange={(event) => setStatus(event.target.value)}>{statuses.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><span>{documentsQuery.data?.page.total ?? 0} documentos</span></div><ErrorMessage error={documentsQuery.error} />{documentsQuery.isLoading && <p className="loading">Cargando documentos…</p>}<div className="table-wrap"><table><thead><tr><th>Archivo</th><th>Estado</th><th>Asociaciones</th><th>Fecha de carga</th><th>Resultado</th></tr></thead><tbody>{documents.map((document) => <tr key={document.public_id}><td><Link to={`/documents/${document.public_id}/review`}>{document.original_filename}</Link></td><td><DocumentStatusBadge status={document.status as DocumentStatus} /></td><td>{document.association_count}</td><td>{dateTime(document.uploaded_at)}</td><td>{document.processing_error || (document.status === "MATCHED" ? "Procesado correctamente" : "-")}</td></tr>)}{!documentsQuery.isLoading && documents.length === 0 && <tr><td colSpan={5}>No hay documentos para este filtro.</td></tr>}</tbody></table></div></section>;
}
