...
export function DocumentsPage() {
  ...
  return <section className="card">
    ...
    <div className="table-wrap"><table><thead><tr><th>Archivo</th><th>Estado</th><th>Asociaciones</th><th>Fecha de carga</th><th>Resultado</th></tr></thead><tbody>{documents.map((document) => { const result = document.processing_error || (document.status === "MATCHED" ? "Procesado correctamente" : null); return <tr key={document.public_id}><td><Link to={`/documents/${document.public_id}/review`}>{document.original_filename}</Link></td><td><DocumentStatusBadge status={document.status as DocumentStatus} /></td><td>{document.association_count}</td><td>{dateTime(document.uploaded_at)}</td><td>{result ? <DocumentResultIndicator message={result} error={Boolean(document.processing_error)} /> : "-"}</td></tr>; })}{!documentsQuery.isLoading && documents.length === 0 && <tr><td colSpan={5}>No hay documentos para este filtro.</td></tr>}</tbody></table></div>
