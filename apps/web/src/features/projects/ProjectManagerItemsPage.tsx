...
export function ProjectManagerItemsPage() {
  ...
  return <>
    ...
    <section className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Obra</th><th>Ítem</th><th>Observación</th><th>Clasificación</th><th>Tipo</th><th>Causa</th><th>Documento</th></tr></thead><tbody>{items.map((item) => <tr key={item.public_id}><td>{item.project_id} · {item.project_name}</td><td>{item.id}</td><td>{item.notes}</td><td>{item.classification}</td><td>{item.item_type}</td><td>{item.failure_cause?.display_name_es || "-"}</td><td>{item.document ? <button className="document-link" type="button" disabled={openingDocument === item.document.public_id} onClick={() => void openDocument(item.document!.public_id)}>{openingDocument === item.document.public_id ? <LoadingIndicator label="Abriendo…" compact /> : `${item.document.original_filename} · ${item.document.status}`}</button> : "Sin documento"}</td></tr>)}{!itemsQuery.isLoading && items.length === 0 && <tr><td colSpan={7}>No hay ítems para este gerente con los filtros seleccionados.</td></tr>}</tbody></table></div>
