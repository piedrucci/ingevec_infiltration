...
export function ProjectManagerItemsPage() {
  ...
  return <>
    ...
    <section className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Obra</th><th>Observación</th><th>Clasificación</th><th>Tipo</th><th>Causas</th><th>Estado</th><th>Documento</th><th /></tr></thead><tbody>{items.map((item) => <tr key={item.public_id}><td>{item.project_id} · {item.project_name}</td><td>{item.notes}</td><td>{item.classification}</td><td>{item.item_type}</td><td>{item.failure_causes.length ? item.failure_causes.map((cause) => <span className="cause-tag" key={cause.code}>{cause.display_name_es}</span>) : "Sin causa"}</td><td><span className={`reconciliation-status ${item.reconciliation_status === "RECONCILED" ? "reconciliation-status-reconciled" : "reconciliation-status-pending"}`}>{item.reconciliation_status === "RECONCILED" ? "Conciliado" : "Pendiente"}</span></td><td>{item.document ? <button className="document-link document-icon-link" type="button" aria-label={`Abrir documento ${item.document.original_filename}`} title={`${item.document.original_filename} · ${item.document.status}`} disabled={openingDocument === item.document.public_id} onClick={() => void openDocument(item.document!.public_id)}>{openingDocument === item.document.public_id ? <LoadingIndicator label="Abriendo…" compact /> : <svg aria-hidden="true" viewBox="0 0 32 36" focusable="false"><path d="M4 1h16l8 8v26H4z" /><path d="M20 1v9h8" /><text x="7" y="26">PDF</text></svg>}</button> : "Sin documento"}</td><td><Link className="manager-link" to={`/items/${item.public_id}/evaluation?returnTo=${encodeURIComponent(`${location.pathname}${location.search}`)}`}>{item.reconciliation_status === "RECONCILED" ? "Editar" : "Evaluar"}</Link></td></tr>)}{!itemsQuery.isLoading && items.length === 0 && <tr><td colSpan={8}>No hay ítems para este gerente con los filtros seleccionados.</td></tr>}</tbody></table></div>
