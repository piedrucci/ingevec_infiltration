...
export function ProjectsPage() {
  ...
  return <>
    ...
    {selectedProject && <section className="card"><div className="section-title"><div><p className="eyebrow">OBRA {selectedProject.id}</p><h2>{selectedProject.name}</h2></div><span>Recepción: {formatDate(selectedProject.municipal_reception_date)}</span></div><div className="table-wrap"><table><thead><tr><th>Ítem</th><th>Observación</th><th>Clasificación</th><th>Tipo</th><th>Causas</th><th>Documento</th></tr></thead><tbody>{items.map((item) => <tr key={item.public_id}><td>{item.id}</td><td>{item.notes}</td><td>{item.classification}</td><td>{item.item_type}</td><td>{item.failure_causes.length ? item.failure_causes.map((cause) => <span className="cause-tag" key={cause.code}>{cause.display_name_es}</span>) : "Sin causa"}</td><td>{item.document ? <button className="document-link" type="button" disabled={openingDocument === item.document.public_id} onClick={() => void openDocument(item.document!.public_id)}>{openingDocument === item.document.public_id ? <LoadingIndicator label="Abriendo…" compact /> : `${item.document.original_filename} · ${item.document.status}`}</button> : "Sin documento"}</td></tr>)}</tbody></table></div></section>}
  </>;
}
