...
import { createFailureCause, replacePostventaItemFailureCauses } from "../../api";
...
export function ItemEvaluationPage() {
  ...
  return <section className="review-layout">
    <section className="card"><div className="section-title"><div><Link className="back-link" to="/items/evaluation">← Volver a evaluación</Link><p className="eyebrow">OBRA {item.project_id} · ÍTEM {item.id}</p><h2>{item.project_name}</h2></div><span className={`reconciliation-status ${item.reconciliation_status === "RECONCILED" ? "reconciliation-status-reconciled" : "reconciliation-status-pending"}`}>{item.reconciliation_status === "RECONCILED" ? "Conciliado" : "Pendiente"}</span></div><div className="detail-grid"><div className="detail-grid-full"><strong>Observación</strong><p>{item.notes}</p></div><div><strong>Clasificación</strong><p>{item.classification}</p></div><div><strong>Tipo</strong><p>{item.item_type}</p></div><div><strong>PDF asociado</strong><p>{item.has_document ? (item.document ? <Link to={`/documents/${item.document.public_id}/review`}>{item.document.original_filename}</Link> : "Sí") : "No"}</p></div></div></section>
