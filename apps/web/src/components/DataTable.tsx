...
export function DataTable<TData extends Record<string, any>>({ data, columns, sorting = [], onSortingChange, getRowId, selectedRowId, onRowClick, emptyMessage = "No hay datos." }: DataTableProps<TData>) {
  ...
  return <div className="table-wrap"><table><thead>{table.getHeaderGroups().map((headerGroup) => <tr key={headerGroup.id}>{headerGroup.headers.map((header) => {
  ...
  })}</tr>)}</thead><tbody>{table.getRowModel().rows.map((row) => <tr key={row.id} className={selectedRowId === row.id ? "selected" : ""} onClick={() => onRowClick?.(row)}>{row.getVisibleCells().map((cell) => <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}{!table.getRowModel().rows.length && <tr><td colSpan={columns.length}>{emptyMessage}</td></tr>}</tbody></table></div>;
}
